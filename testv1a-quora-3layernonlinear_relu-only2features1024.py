...
# paths
parser.add_argument("--nlipath", type=str, default='/home/ubuntu/cs230_project/dataset/SNLI', help="NLI data path (SNLI or MultiNLI)")
...
class NonLinear(nn.Module):
    def __init__(self):
        ...
        self.first_linear = nn.Linear(self.input_dim, self.embed_dim)
        self.first_relu = nn.functional.relu(self.first_linear)
        self.second_linear = nn.Linear(self.first_relu,??)
        self.second_relu = nn.functional.relu(self.second_linear)
        self.third_linear = nn.Linear(??,??)
    def forward(self):
        x = self.first_linear
'''
class NLINet(nn.Module):
    ...
    def forward(self, s1, s2):
        ...
        u = self.encoder(s1)
        v = self.encoder(s2)
        #, torch.abs(u-v), u*v change feature size for classifiers
